...
- `ensureSeedCategories` (SPEC-01) mantiene su propia lista de categorías en lugar de usar `makeSeedCategories` de `src/domain/seed.ts`; hasta este bloque `makeSeedCategories` sólo la usaban las pruebas (otro caso del patrón de T-072, sin efecto visible porque las dos listas son idénticas). `clearAllData` usa la del dominio. Unificarlas es un refactor pequeño fuera del alcance de SPEC-07.
